...
## 9. Управление Docker-образами и очистка диска

Пересборка через `docker compose up -d --build` оставляет слои предыдущих образов и build cache. Это ускоряет будущие сборки, но постепенно занимает диск. На минимальном сервере полезно периодически освобождать место:
...
- Удалить только «висячие» (dangling) образы, которые уже не используются контейнерами:

```bash
docker image prune -f
```

- Полностью удалить все неиспользуемые образы, контейнеры, сети и (опционально) тома:

```bash
docker system prune -f          # без томов
# или, если уверены, что неиспользуемые тома можно удалить
# docker system prune -f --volumes
```

- Очистить cache сборщика, если он разбух после частых билдов:

```bash
docker builder prune -f
```

Рекомендуемая практика после деплоя на маленьком сервере:
...
Это сохраняет только свежесобранные образы и оставляет минимум мусора. Если место снова заканчивается, повторно выполните `docker system df`, оцените, что именно занимает больше всего, и примените более агрессивную очистку.
