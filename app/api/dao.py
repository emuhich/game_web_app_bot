...
class ApplicationDAO(BaseDAO):
    ...
    @classmethod
    async def cancel_application(cls, application_id: int) -> bool:
        async with async_session_maker() as session:
            result = await session.execute(select(cls.model).filter_by(id=application_id))
            ...
            try:
                ...
                print(f"Error cancelling application {application_id}: {e}")
                return False

    @classmethod
    async def reschedule_application(cls, application_id: int, new_date: date, new_time: time) -> bool:
        async with async_session_maker() as session:
            result = await session.execute(select(cls.model).filter_by(id=application_id))
            app_obj = result.scalar_one_or_none()
            if app_obj is None:
            ...
            try:
                ...
                await session.rollback()
                print(f"Error rescheduling application {application_id}: {e}")
                return False
